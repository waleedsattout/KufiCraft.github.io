import { Kufi } from "./kufi";

// Expose functions to global scope
declare global {
    interface Window {
        getCircularCoor: (board: Kufi, _x: number, _y: number) => { innerRadius: number; outerRadius: number; startAngle: number; endAngle: number; };
        drawCircularRect: (board: Kufi, _x: number, _y: number) => SVGPathElement;
        drawCircularLine: (board: Kufi) => boolean;
        drawCircularArch: (board: Kufi, el1: HTMLElement, el2: HTMLElement, bool: boolean) => boolean;
    }
}
 ...
 */
export function getCircularCoor(board: Kufi, _x: number, _y: number): { innerRadius: number; outerRadius: number; startAngle: number; endAngle: number; } {
    ...
    let radialStep: number, angularStep: number;
    let innerRadius: number, outerRadius: number, startAngle: number, endAngle: number;
 ...
 */
export function drawCircularRect(board: Kufi, _x: number, _y: number): SVGPathElement {
    let el = document.createElementNS("http://www.w3.org/2000/svg", "path");
    let { innerRadius, outerRadius, startAngle, endAngle } = getCircularCoor(board, _x, _y);

    el.dataset.innerRadius = innerRadius.toString();
    el.dataset.outerRadius = outerRadius.toString();
    el.dataset.startAngle = startAngle.toString();
    el.dataset.endAngle = endAngle.toString();
 ...
 */
export function drawCircularLine(board: Kufi): boolean {
    ...
    el.setAttribute("stroke-width", (board.rect / 2).toString());
 ...
 */
export function drawCircularArch(board: Kufi, el1: HTMLElement, el2: HTMLElement, bool: boolean = false): boolean {
    const centerX = 500; // Fixed to SVG viewBox center
    const centerY = 500; // Fixed to SVG viewBox center
    ...
    const innerRadius = Math.min(parseFloat(seg1.innerRadius!), parseFloat(seg2.innerRadius!));
    const outerRadius = Math.max(parseFloat(seg1.outerRadius!), parseFloat(seg2.outerRadius!));
    const startAngle = Math.min(parseFloat(seg1.startAngle!), parseFloat(seg2.startAngle!));
    const endAngle = Math.max(parseFloat(seg1.endAngle!), parseFloat(seg2.endAngle!));
